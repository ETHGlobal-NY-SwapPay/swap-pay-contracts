...
task('mint-tokens', 'Mints tokens using Multicall3')
	...
	.setAction(async ({ amount, to }, hre) => {
		...
		const multicall3 = await viem.getContractAt('Multicall3', multicallAddress)

		const aggregateTx = await multicall3.write.aggregate3([calls], {
		...
		await publicClient.waitForTransactionReceipt({ hash: aggregateTx })

		console.log(`✅ All tokens minted via Multicall3. Tx: ${aggregateTx}`)
	})
